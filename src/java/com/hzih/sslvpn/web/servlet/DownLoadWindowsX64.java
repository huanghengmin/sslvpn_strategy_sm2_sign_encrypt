...
public class DownLoadWindowsX64 extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private Logger logger = Logger.getLogger(DownLoadWindowsX64.class);
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            msg = "下载Windows 64位 SSLVPN 客户端成功"+request.getRemoteAddr();
            logger.info(msg+",时间:"+new Date());
        } catch (Exception e) {
            e.printStackTrace();
            msg = "下载Windows 64位 SSLVPN 客户端失败"+request.getRemoteAddr();
            logger.error("下载Windows 64位 SSLVPN 客户端失败"+request.getRemoteAddr(), e);
        }
